...
blockListBX = (filterLens nonBlankLines) `Compose` (minEditDistBX blockBX createBlock) -- `Compose` checkBX
    ...
    Case [ -- Case: AbsPara
         ...
         , $(normalSV [p| UnorderedList _ |] [p| AbsUnorderedList _ |] [p| UnorderedList _ |])
           ==> $(update [p| UnorderedList items |] [p| AbsUnorderedList items |]
                        [d| items = minEditDistBX unorderedListItemBX createListItem |])
         ...
         , $(normalSV [p| OrderedList _ |] [p| AbsOrderedList _ |] [p| OrderedList _ |])
           ==> $(update [p| OrderedList items |] [p| AbsOrderedList items |]
                        [d| items = minEditDistBX orderedListItemBX createListItem |])
...
inlineListBX = (minEditDistBX inlineBX createInline) `Compose` concatAbsStrLens
