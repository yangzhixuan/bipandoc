...
genLI' n = do
  ...
  return $ GTreeNode (CTag Block (Left CListItem) (concat attrs) NormalClose) (map wrapInP blocks)
  where wrapInP = \e -> GTreeNode (CTag Block (Left CPara) [] NormalClose) [e]
...
genP' n = do
  ...
  return $ GTreeNode (CTag Block (Left CPara) (concat attrs) NormalClose) inlines
